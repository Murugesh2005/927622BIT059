...
// Removed circular import - will create separate CorrelationHeatmap component
import { StockSelector } from '../components/StockSelector';
import { TimeFrameSelector } from '../components/TimeFrameSelector';
import { useStockData } from '../hooks/useStockData';
import { useLocalStorage } from '../hooks/useLocalStorage';

// Temporary CorrelationHeatmap component until you create the actual one
      ...
      <Box>
        {selectedStocks.length >= 2 ? (
          <Typography variant="body2" color="textSecondary">
            Correlation heatmap for: {selectedStocks.join(', ')}
            {/* Add your actual heatmap implementation here */}
          </Typography>
        ) : (
          <Typography variant="body2" color="textSecondary">
...
export const CorrelationPage: React.FC = () => {
  ...
  const hasValidData = selectedStocks.length >= 2 && 
    selectedStocks.some(ticker => stocksData && stocksData[ticker] && stocksData[ticker].length > 0);
